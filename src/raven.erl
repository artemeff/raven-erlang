...
-export([start/0, stop/0, capture/2, user_agent/0]).
...
start() -> application:ensure_all_started(?APP).
...
-type parameter() :: {stacktrace, [stackframe()]}
                   | {exception, {exit | error | throw, term()}}
                   | {atom(), binary() | integer()}.
-type stackframe() :: {module(), atom(), non_neg_integer() | [term()]}
...
capture(Message, Params0) ->
        ...
        lists:map(
          fun
            ...
            ({exception, {Type, Value}}) ->
              {'sentry.interfaces.Exception', {[{type, Type}, {value, term_to_json_i(Value)}]}};

            ({tags, Tags}) -> {tags, {[{Key, term_to_json_i(Value)} || {Key, Value} <- Tags]}};
            ({extra, Tags}) -> {extra, {[{Key, term_to_json_i(Value)} || {Key, Value} <- Tags]}};
            ({Key, Value}) -> {Key, term_to_json_i(Value)}
          end,
          Params2
        )
      ]
  ...
  httpc:request(
    post,
    {
      ...
      "/api/"
      ++
      binary_to_list(unicode:characters_to_binary(Cfg#cfg.project))
      ++
      "/store/",
      Headers,
      "application/json",
...
get_tags() -> application:get_env(?APP, tags, []).
...
event_id_i() ->
  ...
  <<UUID : 128>> = <<U0 : 32, U1 : 16, 4 : 4, U2 : 12, 2 : 2, U3 : 30, U4 : 32>>,
...
term_to_json_i(Term) when is_binary(Term); is_atom(Term) -> Term;
term_to_json_i(Term) -> iolist_to_binary(io_lib:format("~120p", [Term])).
